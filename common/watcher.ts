...
async function itemCheck(product: LinkItem, bot: Client) {
	...
	const underPriceLimit = product.priceLimit ? newPrice !== -1 && newPrice <= product.priceLimit : false
	...
	if (underPriceLimit && (product.lastPrice !== -1 && product.lastPrice > newPrice)) {
